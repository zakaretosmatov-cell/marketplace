...
export default function OrderDetailPage() {
  ...
  // Build timeline: merge statusHistory with ALL_STATUSES progress
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: "1.5rem", alignItems: "start" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
            ...
            <div style={{ border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "1.5rem", background: "var(--bg-card)" }}>
              <h2 style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", marginBottom: "1.5rem" }}>Order Timeline</h2>
                      ...
                      return (
                        <div key={s} style={{ display: "flex", gap: "1rem", alignItems: "flex-start" }}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: isCompleted ? m.bg : "var(--bg-tertiary)", color: isCompleted ? m.color : "var(--text-tertiary)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: isCurrent ? "2px solid " + m.color : "2px solid transparent", transition: "all 0.2s" }}>{m.icon}</div>
                            {!isLast && <div style={{ width: "2px", height: "36px", background: isCompleted ? "var(--border-color)" : "var(--border-color)", margin: "2px 0" }} />}
                          </div>
                          <div style={{ flex: 1, paddingTop: "0.3rem", paddingBottom: isLast ? 0 : "1.25rem" }}>
            ...
            <div style={{ border: "1px solid var(--border-color)", borderRadius: "0.5rem", overflow: "hidden", background: "var(--bg-card)" }}>
              <div style={{ padding: "1rem 1.25rem", borderBottom: "1px solid var(--border-color)", background: "var(--bg-secondary)" }}>
                <h2 style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)" }}>Items ({order.items.length})</h2>
              </div>
              {order.items.map((item, i) => (
                <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "1rem 1.25rem", borderBottom: i < order.items.length - 1 ? "1px solid var(--border-color)" : "none" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                    <div style={{ width: "40px", height: "40px", borderRadius: "0.375rem", background: "var(--bg-tertiary)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  ...
                  <p style={{ fontWeight: 600, fontSize: "0.9rem" }}>${(item.price * item.quantity).toFixed(2)}</p>
                </div>
              ))}
              <div style={{ padding: "1rem 1.25rem", borderTop: "1px solid var(--border-color)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <p style={{ fontWeight: 700, fontSize: "1.1rem" }}>${order.totalAmount.toFixed(2)}</p>
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
            ...
            <div style={{ border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "1.25rem", background: "var(--bg-card)" }}>
              <h2 style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>Shipping Address</h2>
              <div style={{ display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
                <MapPin size={16} color="var(--text-tertiary)" style={{ marginTop: "0.15rem", flexShrink: 0 }} />
                <p style={{ fontSize: "0.875rem", lineHeight: 1.6 }}>{order.shippingAddress}</p>
              </div>
              {order.trackingNumber && (
                <div style={{ marginTop: "0.75rem", padding: "0.6rem 0.75rem", background: "var(--bg-secondary)", borderRadius: "0.375rem", fontSize: "0.8rem" }}>
            ...
            {/* Order info */}
            <div style={{ border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "1.25rem", background: "var(--bg-card)" }}>
              <h2 style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>Order Info</h2>
              {[
                { label: "Order ID", value: order.id.slice(-10).toUpperCase() },
                { label: "Created", value: createdDate.toLocaleDateString() },
                { label: "Status", value: meta.label },
                { label: "Items", value: order.items.length + " item(s)" },
              ].map(row => (
                <div key={row.label} style={{ display: "flex", justifyContent: "space-between", padding: "0.4rem 0", borderBottom: "1px solid var(--border-color)", fontSize: "0.875rem" }}>
            ...
            {canManage && order.status !== "delivered" && order.status !== "cancelled" && (
              <div style={{ border: "1px solid var(--border-color)", borderRadius: "0.5rem", padding: "1.25rem", background: "var(--bg-card)" }}>
                <h2 style={{ fontSize: "0.875rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em", color: "var(--text-secondary)", marginBottom: "0.75rem" }}>Update Status</h2>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                  {ALL_STATUSES.filter(s => s !== order.status).map(s => {
                    const m = STATUS_META[s];
                    return (
                      <button key={s} onClick={() => handleStatusUpdate(s)} disabled={updating} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.6rem 0.9rem", borderRadius: "0.375rem", border: "1px solid var(--border-color)", background: "var(--bg-secondary)", color: "var(--text-primary)", cursor: updating ? "not-allowed" : "pointer", fontSize: "0.875rem", fontWeight: 500, transition: "background 0.15s" }}
                  ...
                  <button onClick={() => handleStatusUpdate("cancelled")} disabled={updating} style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.6rem 0.9rem", borderRadius: "0.375rem", border: "1px solid #fee2e2", background: "#fff5f5", color: "#7f1d1d", cursor: updating ? "not-allowed" : "pointer", fontSize: "0.875rem", fontWeight: 500 }}>
...
'@
Set-Content -Path "src/app/orders/[id]/page.tsx" -Value $detail -Encoding UTF8
Write-Host "order detail written"
